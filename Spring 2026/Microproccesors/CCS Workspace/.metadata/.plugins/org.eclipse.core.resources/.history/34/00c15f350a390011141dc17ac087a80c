//-----------------------------------------------------
// Lucas Jones, EELE 371, Final Project: Day 3, 04/15/2026
//
// Timer: 1,000,000 / 4 = 250,000 / 4 = 62,500
//-----------------------------------------------------

#include <msp430.h> 

//global vars
int S1;
int S2;
int i;
int j;
int S1Loops = 4;
int S2Loops = 8;
int countS1 = 0;
int countS2 = 0;

int main(void)
{
	WDTCTL = WDTPW | WDTHOLD;	// stop watchdog timer
	
	// config 4 leds to p3.0-p3.3 nibble
	P3DIR |= BIT0 | BIT1 | BIT2 | BIT3;
	P3OUT &= ~(BIT0 | BIT1 | BIT2); // all except LSB start off
	P3OUT |= BIT3;                  // LSBit start on

	// config s1 and s2 as inputs
	P4DIR &= ~BIT1;             // S1 as input
	P4REN |= BIT1;              // Enable resistor on S1
	P4OUT |= BIT1;              // Set as pull-UP
	P4IES |= BIT1;               // high-to-low edge

	P2DIR &= ~BIT3;             // S2 as input
	P2REN |= BIT3;              // Enable resostor on S2
	P2OUT |= BIT3;              // Set as pull-UP
	P2IES |= BIT3;               // high-to-low edge

	// turn of gpio
	PM5CTL0 &= ~LOCKLPM5;

	//config timer
	TB0CTL |= TBCLR;            // clear timers and dividers
	TB0CTL |= TBSSEL__SMCLK;    // clock source SMCLK
	TB0CTL |= MC__UP;           // mode up
	TB0CTL |= ID__4;            // divide by 4

	TB0CTL |= TBIDEX__4;        // divide by 4 again

// ----------------------- MAIN LOOP -------------------------
	while(1){
	    S1 = P4IN;               // read p4 to S1 var
	    S1 &= BIT1;              // clears bits in S1except bit 1
	    if(S1 == 0){
	        while(S1Loops > 0 & countS1 < 4){
	            P3OUT = P3OUT >> 1;
	            S1Loops--;
	            //0.25s delay
	            for(i=0; i<0x3FFF; i++){}
	            if(S1Loops == 0){
	                P3OUT &= ~(BIT0 | BIT1 | BIT2); // all except LSB start off
	                P3OUT |= BIT3;                  // LSBit start on
	                S1Loops = 4;
	                countS1++;
	            }

	        }
	        S1Loops = 4;
	    }

	    S2 = P2IN;               // read p4 to S1 var
	    S2 &= BIT3;              // clears bits in S1except bit 1
	    if(S2 == 0){
	        while(S2Loops > 0){
	            P3OUT = P3OUT << 1;
	            S2Loops--;
	            //0.25s delay
	            for(j=0; j<0x3FFF; j++){}
	            if(S2Loops == 0){
	                P3OUT &= ~(BIT0 | BIT1 | BIT2); // all except LSB start off
	                P3OUT |= BIT3;                  // LSBit start on
	                S2Loops = 4;
	                countS2++;
	            }
	        }
	        S2Loops = 4;
	    }

	}
	return 0;
}

